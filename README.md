...
- [x] **Future Date Block Safeguard** — Blocks future dates from being saved as an expense (e.g., `2026-06-10` is rejected with `Expense date cannot be in the future.`), preventing logically impossible future transaction entries.
...
| **Future Date Prevention** | `{"date": "2030-01-01"}` | Action blocked with toast message: `"⚠️ Error: Expense date cannot be in the future."` | `400 Bad Request` | Pydantic `@field_validator('date')` comparison against `datetime.date.today()`. |
